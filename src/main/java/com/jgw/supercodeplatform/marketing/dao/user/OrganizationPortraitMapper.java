...
public interface OrganizationPortraitMapper extends CommonSql{
    ...
    @Select(" SELECT Id,OrganizationId,OrganizationFullName,FileWeight,UnitCodeId FROM marketing_organization_portrait  WHERE OrganizationId = #{OrganizationId} and UnitCodeId= #{UnitCodeId} ")
     ...
     */
    @Select(" SELECT "+selectSqlUnitcode+" FROM marketing_unitcode WHERE TypeId = #{typeId} and  Id not in (select UnitCodeId from marketing_organization_portrait where OrganizationId = #{organizationId}) ")
    List<MarketingUnitcode> getUnSelectedLabel(String organizationId,@Param("typeId")Integer typeId);
